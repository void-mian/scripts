# forked from https://github.com/lxy764139720/Auto_Attendance
import requests
...
class DaKa:
    ...
    def login(self, eai_sess):
        ...
        try:
            ...
            print("中南大学统一登录过程出错", file=sys.stderr)
            try:
                self.sess.cookies.set(
                    "eai-sess", eai_sess, domain="wxxy.csu.edu.cn")
            except:
                print("cookie设置出错", file=sys.stderr)
                exit(1)
    ...
    def get_info(self):
        ...
        jsontext = eval(
            data1[data1.find("{"):data1.rfind(";")].replace(" ", ""))
        geo_text = jsontext['geo_api_info']
        geo_text = geo_text.replace("false", "\"FALSE\"").replace(
...
def main(username, password, eai_sess):
    ...
    res = dk.post()
    if str(res['e']) == '0':
        print('✔为%s同学打卡成功' % dk.info['number'][-3:])
    else:
        print('❌%s' % res['m'], file=sys.stderr)
...
if __name__ == "__main__":
    ...
    main(args.username, args.password, args.cookie)
